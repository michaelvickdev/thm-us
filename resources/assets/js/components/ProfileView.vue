  ...
  <div style="z-index: -1">
    ...
    <div class="col-lg-12 col-xs-12 col-sm-12" id="box-btn-footer">
      <div class="col-lg-6 col-xs-12 col-sm-6">
        <a href="#" class="btn btn-success btn-block" id="btn-light-green"
           v-on:click.prevent="showAdditionalFood = !showAdditionalFood"><i class="fa fa-plus"></i>Add Additional Foods
          Consumed</a>
        <div v-bind:class="[  showAdditionalFood ? '' : 'hidden' ]" style="padding: 20px;">
          I ate <input class="form-control" v-model="additional" style="display: inline; width: 150px" type="text"
                       placeholder="123"> more calories.
          <button class="btn btn-success" @click.prevent="updateAdditional" style="margin-top: -2px"><i
              class="fa fa-check"></i> Confirm
          </button>
        </div>
      </div>
      <div class="col-lg-6 col-xs-12 col-sm-6">
        <a href="#" class="btn btn-success btn-block" id="btn-light-orange"
           v-on:click.prevent="showExercise = !showExercise"><i class="fa fa-plus"></i>Add Exercise</a>
        <div v-bind:class="[  showExercise ? '' : 'hidden' ]" style="padding: 20px;">
          I spent <input class="form-control" v-model="exercise" style="display: inline; width: 150px" type="text"
                         placeholder="123"> calories exercising!
          <button class="btn btn-success" @click.prevent="updateExercise" style="margin-top: -2px"><i
              class="fa fa-check"></i> Confirm
          </button>
        </div>
      </div>
    ...
    <div class="col-lg-12 col-xs-12 col-sm-12" id="box-btn-footer" style="margin-bottom: 20px">
      <div class="col-lg-4 col-xs-12 col-sm-4">
        <div class="supporting-text">
          ...
          <small class="text-muted" style="font-size: 12px">
            {{totalCalories}} + {{additional ? additional : 0}} additional - {{ parseInt(exercise ? exercise : 0) }}
            exercise
          </small>
        </div>
      </div>
      ...
      <div class="col-lg-4 col-xs-12 col-sm-4">
        ...
        <div class="supporting-text-orange" v-if="parseInt(total) <= minGoal">
          <p class="text-center">You are not eating enough. Please mark the meals as complete to get an updated
            calculation.</p>
        </div>
        <div class="supporting-text-green"
             v-if="parseInt(total) > minGoal && parseInt(total) <= parseInt(caloryGoal)*1.1">
          <p class="text-center">Congratulations, You consumed a healthy amount of calories that work with your weight
            goals.</p>
        </div>
